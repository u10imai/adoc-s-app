...
ID発行時に「本番(001〜)」「テスト(1201〜)」を選べるようにし、「ID発行」タブに発行済みID・パスワードの一覧を常時表示するようにしました。

※ テスト用IDは当初201番台で採番していましたが、本番IDの一括発行時に衝突するため、`0017_move_test_subject_codes_to_1000s.sql`で1000番台(既存分は先頭に「1」を付与し1201〜)へ移行しています。
